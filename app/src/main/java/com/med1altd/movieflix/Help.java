...
public class Help {
        ...
        getCurrentDateAndTime(context, new ReturnMultipleResults() {
            ...
            public ArrayList<Object> onSuccessMultipleResults(ArrayList<Object> results) {
                ...
                if (results != null) {
                    ...
                    DatabaseReference
                            databaseReferenceValue = firebaseDatabase.getReference().child(new Help().getCurrentBundleVersion(context)).child("AdMob").child("Statistics").child(dividedDate.get(0)).child(dividedDate.get(1)).child(dividedDate.get(2)).child(time).child("Requests");
    ...
    public void pushStringToFirebase(Context context, String category, String value) {
        ...
        DatabaseReference
                databaseReferenceBasic = firebaseDatabase.getReference().child(category).child(value),
                databaseReferenceSpesific = firebaseDatabase.getReference().child(new Help().getCurrentBundleVersion(context)).child(category).child(value);

        // Add a listener to check if data exists at that location

        databaseReferenceBasic.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.exists()) {
                    ...
                    databaseReferenceBasic.child("Views").setValue(currentTries + 1);

                } else {

                    // If data does not exist, generate a new unique key

                    databaseReferenceBasic.child("Views").setValue(1);
        ...
        databaseReferenceSpesific.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.exists()) {
                    ...
                    databaseReferenceSpesific.child("Views").setValue(currentTries + 1);

                } else {

                    // If data does not exist, generate a new unique key

                    databaseReferenceSpesific.child("Views").setValue(1);
    ...
    String TimeAPIUrl = "https://worldtimeapi.org/api/timezone/Europe/Athens";
